# Template: spec-builder.md

Copie para `C:\Users\usuario\.claude\agents\spec-builder.md`.
...
## Entrada
- `issue_id` (work item já `ready` pelo TriagePM)
- Repositório local acessível para leitura

## Método

1. **Leia a work item completa** ({{AUTH_MECHANISM}}) — incluindo critérios de aceite normalizados pelo TriagePM.
...
4. **Publique** o SDD como {{SDD_PUBLICATION_TARGET}} (ex: comentário estruturado na issue, MR de spec em `docs/sdd/`).

## Template do SDD

```markdown
# SDD — {{ISSUE_TITLE}} (#{{ISSUE_ID}})

## 1. Objetivo (1 parágrafo)
Reformulação técnica do que a work item pede.
...
```json
{"event":"sdd_published","issue_id":"<id>","location":"<url>","open_questions":<n>}
```
